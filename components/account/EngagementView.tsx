  ...
  return (
    <div className="surface-card p-6">
      <div className="flex items-center justify-between">
        ...
        {showCancel && (
          <button type="button" onClick={onDone} aria-label="Cancel" className="focus-ring rounded-lg p-1.5 text-muted-foreground hover:bg-brand-bg"><X className="h-4 w-4" /></button>
        )}
      </div>
      <p className="text-body mt-2 text-muted-foreground">Ask a question or request support. Your tutor replies right here, and you'll get a notification.</p>
...
function Card({ eng }: { eng: EngagementFull }) {
  ...
  function send() {
    ...
    setDraft("");
    start(async () => { const r = await studentPostMessage(eng.id, t); if (!r.success) toast.error(r.message ?? "Couldn't send."); });
  }
  return (
