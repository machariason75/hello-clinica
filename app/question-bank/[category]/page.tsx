...
import { ArrowLeft, ArrowRight, ListChecks, Clock, BarChart3, ChevronRight } from "lucide-react";
...
export default async function QuizCategoryPage({ params }: Params) {
  ...
  const ancestors = await getCategoryAncestors(node.id);
  ...
  const isEmpty = children.length === 0 && quizzes.length === 0 && links.length === 0;

  return (
    <PageTransition>
      <PageHero eyebrow="Question Bank" title={node.title} description={node.overview || node.description} />
      ...
      <Section ariaLabel={node.title}>
        ...
        <nav aria-label="Breadcrumb" className="mb-8 flex flex-wrap items-center gap-1.5 text-sm text-muted-foreground">
          <Link href="/question-bank" className="focus-ring rounded hover:text-coral">
            Question Bank
          </Link>
          {ancestors.slice(0, -1).map((a) => (
            <span key={a.slug} className="flex items-center gap-1.5">
              <ChevronRight className="h-3.5 w-3.5" aria-hidden="true" />
              <Link href={`/question-bank/${a.slug}`} className="focus-ring rounded hover:text-coral">
                {a.title}
              </Link>
            </span>
          ))}
          <ChevronRight className="h-3.5 w-3.5" aria-hidden="true" />
          <span className="font-medium text-deep-blue">{node.title}</span>
        </nav>
        ...
        {isEmpty ? (
          <EmptyState
            ...
            title="New material is on the way"
            description="We're preparing content for this section. Check back soon, or subscribe to be notified."
          />
        ) : (
                ...
                <StaggerGroup className="grid gap-6 md:grid-cols-2">
                  {quizzes.map((quiz) => (
                    <StaggerItem key={quiz.id} className="h-full">
                      <Link
                        href={`/question-bank/${node.slug}/${quiz.slug}`}
