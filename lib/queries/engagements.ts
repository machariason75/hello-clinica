...
export async function getEngagementForStudent(id: string, studentId: string): Promise<EngagementFull | null> {
  return (prisma as any).engagement.findFirst({ where: { id, studentId }, include: { steps: { orderBy: { order: "asc" } }, messages: { orderBy: { createdAt: "asc" } } } });
}
export async function getAllEngagements(): Promise<EngagementFull[]> {
  return (prisma as any).engagement.findMany({ orderBy: { updatedAt: "desc" }, include: { steps: { orderBy: { order: "asc" } }, messages: { orderBy: { createdAt: "asc" } }, student: { select: { name: true, email: true } } } });
}
